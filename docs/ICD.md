...
The public Inbound API uses Bearer authentication when configured.
...
If inbound authentication is enabled and the header value does not match the configured token, the API returns:
...
If inbound authentication is not configured, requests are accepted without the `Authorization` header.
...
- validate inbound input;
- persist the inbound event;
- enqueue asynchronous processing;
- return immediate acceptance or rejection.

`status = ok` means the event was accepted for asynchronous processing. It does not guarantee that a customer-visible response has already been sent.
...
- `ticket_id`: required, string. Is assigned by the external caller that opens or owns the case.
- `message_text`: required, non-empty string.
- `payment_data`: required, non-empty JSON object.
- `project_id`: optional string. In v1 it is accepted and persisted, but currently ignored by backend decision logic.
...
### Lifecycle Rejection Response

If the referenced ticket is already finalized and has already been escalated to a human, the inbound message is rejected instead of being re-accepted.
...
### Dashboard / UI Testing Usage Pattern

Recommended flow for an external UI testing tool:

1. Call `POST /api/v1/inbound/messages` with a synthetic customer message.
2. Poll the database in read-only mode for:
   - the new `inbound_events` row;
   - new `outbound_messages` rows;
   - updates to the `tickets` row and its `messages` JSON.
3. Render:
